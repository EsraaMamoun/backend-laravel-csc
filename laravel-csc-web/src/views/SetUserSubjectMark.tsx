...
export default function SetUserSubjectMark(props: SetUserSubjectMarkProps) {
  ...
  useEffect(() => {
    ...
    axiosClient.get("/users").then((response) => {
      setState((prevState) => ({ ...prevState, users: response.data.data }));
    });
  }, [props.isOpen]);
  ...
  const fetchSubjectsForUser = (userId: number) => {
    axiosClient.get(`/user-subject/marks/${userId}`).then((response) => {
      console.log("response.data?.subject", response.data.data);

      let subjectsWithNullMarks: {
        subject: any;
      }[] = [];

      if (response.data.data.length > 0) {
        subjectsWithNullMarks = response.data.data.filter(
          (subject: any) => subject?.mark === null
        );
      }

      console.log("subjectsWithNullMarks", subjectsWithNullMarks);

      if (subjectsWithNullMarks.length > 0) {
        setState((prevState) => ({
          ...prevState,
          submitDisabled: false,
          subjects: subjectsWithNullMarks,
          errors: null,
        }));
      } else {
        setState((prevState) => ({
